...
# Initializing with mutual info and covariance
# Using 16 greyWolves

class BMOGWO:

    def __init__(self, dataset, classifier, greyWolvesNum=16, maxIt=100, archiveSize=50,
    ...
    def initialize(self):
        ...
        initial_wolves = Initializer(self.dataset)

        len_initial_wolves = len(initial_wolves) 
        self.greyWolvesNum

        for i in range(self.greyWolvesNum - len_initial_wolves):

          initial_wolves[len_initial_wolves + i] = (np.random.uniform(size = self.dim) >= 0.5) * 1
          while(np.count_nonzero(initial_wolves[len_initial_wolves + i])==0):
            initial_wolves[len_initial_wolves + i] = (np.random.uniform(size = self.dim) >= 0.5) * 1
        ...
        for i in range(self.greyWolvesNum):
            ...
            greyWolves[i].position = initial_wolves[i]
